$(function() {
	$('#ProcessPageEdit').on('click', '.url-upload-toggle', function(e) {
		...
		var $this = $(this),
				$fileList = $('.InputfieldFileList'),
				$progressItem = $('<li class="InputfieldFile ui-widget AjaxUpload"><p class="InputfieldFileInfo ui-widget ui-widget-header InputfieldItemHeader"></p></li>'),
		...
		//MP todo: on fail tudi sprocesiraj
		...
		function upload(response) {
			for(var n = 0; n < response.length; n++) {
				...
				if(r.error) {
					...
					//MP don't need this, we will always insert
					if(r.overwrite) {
				...
				setTimeout(function() {
					//MP $markup je undefined ce je error
